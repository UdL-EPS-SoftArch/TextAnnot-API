    ...
    public void itsTagsHaveTheCorrectParentChildRelationship() throws Throwable {
        stepDefs.mockMvc.perform(
                get("/tagHierarchies/" + 1 + "/tags")
                        ...
                        .with(AuthenticationStepDefs.authenticate()))
                .andDo(print())
                .andExpect(jsonPath("$.roots[0].name", is("root1")))
                .andExpect(jsonPath("$.roots[0].children[0].name", is("son_of_root_1")))
                .andExpect(jsonPath("$.roots[0].children[0].children[0].name", is("grandchild_of_root1")));
    }
}
